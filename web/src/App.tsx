...
function CommandsView({ organizationId, groups, devices, onComplete }: { organizationId: string; groups: Group[]; devices: Device[]; onComplete: (message: string, tone?: 'success' | 'error') => void }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); try { const payload = type === 'show_notification' ? { title, message } : {}; const result = await api.createCommand({ type, [`${targetKind}_id`]: targetId, payload, expires_in_seconds: 300 }); onComplete(`命令 #${result.cursor} 已进入下发队列`) } catch (error) { onComplete(error instanceof Error ? error.message : '下发失败', 'error') } }
  const targets = targetKind === 'group' ? groups : devices
  return <div className="command-layout"><section className="form-section command-form"><form onSubmit={submit}><div className="segmented"><button type="button" className={targetKind === 'group' ? 'selected' : ''} onClick={() => { setTargetKind('group'); setTargetId('') }}>分组</button><button type="button" className={targetKind === 'device' ? 'selected' : ''} onClick={() => { setTargetKind('device'); setTargetId('') }}>单台设备</button></div><label>目标<select value={targetId} onChange={(event) => setTargetId(event.target.value)}><option value="">选择目标</option>{targets.map((target) => <option key={target.id} value={target.id}>{target.name}</option>)}</select></label><label>操作<select value={type} onChange={(event) => setType(event.target.value)}><option value="refresh_status">立即刷新状态</option><option value="restart_app">重启 Class Widgets</option><option value="upload_diagnostics">上传诊断信息</option><option value="show_notification">显示通知</option></select></label>{type === 'show_notification' && <><label>通知标题<input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>通知内容<textarea value={message} onChange={(event) => setMessage(event.target.value)} /></label></>}<div className="form-actions"><span>命令将在设备下次 10 秒轮询时获取。</span><button className="primary" disabled={!targetId}><Code24Regular />下发命令</button></div></form></section><CommandHistory commands={commands} groups={groups} devices={devices} /></div>
}

function CommandHistory({ commands, groups, devices }: { commands: CommandRecord[]; groups: Group[]; devices: Device[] }) {
  const labels: Record<string, string> = { refresh_status: '刷新状态', restart_app: '重启应用', upload_diagnostics: '上传诊断', show_notification: '显示通知' }
